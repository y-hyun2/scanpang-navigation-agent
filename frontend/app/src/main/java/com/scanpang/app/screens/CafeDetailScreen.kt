...
fun CafeDetailScreen(
    ...
    LaunchedEffect(Unit) { viewModel.searchConvenience(category = "cafe") }

    val context = LocalContext.current
    val place = remember(convenienceResult) {
        if (convenienceResult?.category == "cafe") {
            convenienceResult?.facilities?.firstOrNull()?.toPlace("카페")
        } else null
    } ?: DummyData.cafes.first()
    ...
    val menus = remember(place.id) { DummyData.cafeRepresentativeMenus[place.id].orEmpty() }
    val bookmark = rememberDetailBookmark(
        ...
        tags = place.tags,
        target = SavedPlaceNavTarget.Cafe,
    )
    ...
    Column(
        ...
        Column(
            ...
            if (menus.isEmpty()) {
                DetailIntroBody(text = "등록된 대표 메뉴가 없습니다.")
            } else {
