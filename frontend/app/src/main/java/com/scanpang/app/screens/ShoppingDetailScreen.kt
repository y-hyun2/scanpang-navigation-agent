...
private fun parkingSummary(place: Place): String =
    place.tags.firstOrNull { it.contains("주차") } ?: "정보 없음"
...
fun ShoppingDetailScreen(
    ...
    LaunchedEffect(Unit) { viewModel.searchConvenience(category = "shopping") }

    val place = remember(convenienceResult) {
        if (convenienceResult?.category == "shopping") {
            convenienceResult?.facilities?.firstOrNull()?.toPlace("쇼핑")
        } else null
    } ?: DummyData.shoppingPlaces.first()
    ...
    val bookmark = rememberDetailBookmark(
        ...
        tags = place.tags,
        target = SavedPlaceNavTarget.Shopping,
    )
    ...
    Column(
        ...
        Column(
            ...
            DetailInfoLine(Icons.Rounded.LocalParking, "주차여부", parkingSummary(place))
