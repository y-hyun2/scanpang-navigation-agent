...
private fun departmentTags(place: Place): List<String> =
    place.tags.filter { !it.contains("외국어") && !it.contains("응급") }

private fun emergencyLine(place: Place): String =
    if (place.tags.any { it.contains("응급") }) "응급실 운영 (자세한 시간은 병원 안내)" else "응급실 미운영 또는 제한 (병원 확인)"
...
fun HospitalDetailScreen(
    ...
    LaunchedEffect(Unit) { viewModel.searchConvenience(category = "hospital") }

    val context = LocalContext.current
    val place = remember(convenienceResult) {
        if (convenienceResult?.category == "hospital") {
            convenienceResult?.facilities?.firstOrNull()?.toPlace("병원")
        } else null
    } ?: DummyData.hospitalPlaces.first()
    ...
    val bookmark = rememberDetailBookmark(
        ...
        tags = place.tags,
        target = SavedPlaceNavTarget.Hospital,
    )
    ...
    Column(
        ...
        Column(
            ...
            DetailScreenDivider()
            DetailSectionHeader(title = "진료과목")
            DetailFacilityTagRow(tags = departmentTags(place))
            DetailScreenDivider()
            ...
            DetailInfoLine(Icons.Rounded.LocalHospital, "응급실 여부", emergencyLine(place))
