...
@Composable
fun ConvenienceStoreDetailScreen(
    ...
    LaunchedEffect(Unit) { viewModel.searchConvenience(category = "convenience_store") }

    val context = LocalContext.current
    val place = remember(convenienceResult) {
        if (convenienceResult?.category == "convenience_store") {
            convenienceResult?.facilities?.firstOrNull()?.toPlace("편의점")
        } else null
    } ?: DummyData.convenienceStores.first()
    ...
    val bookmark = rememberDetailBookmark(
        ...
        tags = place.tags,
        target = SavedPlaceNavTarget.ConvenienceStore,
    )
    ...
    Column(
        ...
        Column(
            ...
            DetailFacilityTagRow(tags = place.tags.distinct())
            DetailScreenDivider()
