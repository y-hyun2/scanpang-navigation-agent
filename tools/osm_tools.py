...
async def find_building_by_osm(lat: float, lng: float, heading: float) -> str:
    """
    ARCore VPS가 제공하는 GPS 위치(lat, lng)와 카메라 방위각(heading)으로
    Overpass API에서 OSM 건물 폴리곤을 가져와 레이캐스팅 → 바라보는 건물명 반환.
    인식 실패 시 빈 문자열 반환.
    """
...
def _raycast(lat: float, lng: float, heading: float, buildings: list) -> str:
    """
    heading(0=북, 90=동, 180=남, 270=서) 방향으로 300m 레이를 쏴서
    가장 먼저 교차하는 OSM 건물의 이름을 반환.
    """
    ...
    if best_name:
        print(f"[OSM] 레이캐스팅 결과: {best_name!r} (heading={heading:.1f}°)")
    else:
